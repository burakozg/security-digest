...
# Deploy one or more instances to the NAS, building the image NATIVELY on the NAS.
#
#   ./deploy-native.sh                          every instance under instances/
#   ./deploy-native.sh --instance news          just that one
#   ./deploy-native.sh --instance news --instance security   both, explicitly
#   ./deploy-native.sh --skip-run               (any of the above) leave containers alone
#
# Alternative to deploy.sh: instead of cross-compiling for linux/amd64 on the dev
# machine (buildx + QEMU emulation) and transferring a large image tar, this
# rsyncs the source to the NAS and runs `docker build` there -- whatever
# architecture its Docker daemon actually is. No --platform flag, no emulation,
# faster and more reliable for a pure-Python image.
#
# Safety: source is synced to a SEPARATE, disposable build directory, never to an
# instance directory. `rsync --delete` only ever runs against that build
# directory, so it cannot touch a seen-store, history or digest output.
#
# Deploying everything is the default because the alternative -- defaulting to
# one instance -- means a plain `./deploy-native.sh` silently leaves the others running
...
# Disposable build checkout, shared by every instance (the image has no
# instance-specific content, so it is built once per deploy).
TARGET_BUILD_PATH="/share/Container/security-digest-build"
...
# ---------------------------------------------------------------------------
# 2. Sync the source and build the image ONCE on the NAS. Every instance runs
#    the same image, so building per instance would repeat identical work.
# ---------------------------------------------------------------------------
echo "==> Checking NAS processor architecture..."
echo "    uname -m:       $(ssh -p "${SSH_PORT}" "${TARGET}" "uname -m")"
echo "    docker reports: $(ssh -p "${SSH_PORT}" "${TARGET}" "${DOCKER_BIN} info --format '{{.Architecture}}'" 2>/dev/null || echo unknown)"
echo "    (Built natively on the NAS -- informational; no --platform or emulation needed.)"

echo "==> Syncing source to ${TARGET}:${TARGET_BUILD_PATH} (disposable -- never an instance dir)..."
ssh -p "${SSH_PORT}" "${TARGET}" "mkdir -p '${TARGET_BUILD_PATH}'"
rsync -avz --delete \
  -e "ssh -p ${SSH_PORT}" \
  --exclude '.venv' \
  --exclude '.git' \
  ...
  --exclude 'output/' \
  --exclude '*.tar' \
  ./ "${TARGET}:${TARGET_BUILD_PATH}/"

echo "==> Building image natively on the NAS..."
ssh -p "${SSH_PORT}" "${TARGET}" "cd '${TARGET_BUILD_PATH}' && ${DOCKER_BIN} build -t ${IMAGE_NAME} ."
